...
def build_methodology_pdf():
    ...
    # v93.24: dropped the "vX.Y" app-release tag from this document -- methodology.pdf is
    # a static file regenerated by hand only when the METHODOLOGY content itself changes,
    # not on every app deploy, so stamping it with the app's current patch version looked
    # "stale"/broken the moment an unrelated app-only change (e.g. a bug fix with no
