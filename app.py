...
PROFILES = {
    ...
    "delhaize": ("Delhaize", "Food retail and supermarkets", "Medium", "Food retail can involve franchise labour, supplier pressure, agricultural labour, affordability claims and value-chain human-rights risks."),
    ...
    "sodexo": ("Sodexo", "Catering, facilities management and outsourced services", "High", "Outsourced catering and facilities work creates exposure to wages, workloads, subcontracting, vulnerable workers and well-being claims."),
    ...
    "bnpparibas": ("BNP Paribas Fortis", "Banking and sustainable finance", "Medium", "Responsible-finance and human-rights claims can be sensitive where financing decisions affect high-risk sectors or geographies."),
    "bnp-paribas": ("BNP Paribas Fortis", "Banking and sustainable finance", "Medium", "Responsible-finance and human-rights claims can be sensitive where financing decisions affect high-risk sectors or geographies."),
    "brusselsairlines": ("Brussels Airlines", "Aviation and passenger transport", "Medium", "Aviation claims may involve passenger care, accessibility, labour relations, disruption remediation and contractor/service-chain exposure."),
    "flixbus": ("FlixBus", "Platform-based passenger transport", "Medium", "Platform-based transport creates exposure to partner/operator standards, driver conditions, passenger rights, accessibility and safety claims."),
    "puratos": ("Puratos", "B2B food ingredients", "Medium", "Food-ingredient sourcing creates exposure to farmer livelihoods, cocoa/palm-oil supply chains, child-labour monitoring and traceability claims."),
    "solvay": ("Solvay", "Chemicals and industrial manufacturing", "Medium", "Industrial chemicals can involve safety, contractors, social dialogue, restructuring, community exposure and licence-to-operate concerns."),
    "utexbel": ("Utexbel", "Textiles and industrial manufacturing", "Medium", "Textile production and circularity claims can involve worker, contractor, safety, supplier and community expectations."),
}

SECTOR_RULES = [
    ("High", ["fast fashion", "apparel", "garment", "fashion", "clothing", "discount", "supermarket", "grocery", "food retail", "catering", "facilities", "outsourced", "platform", "delivery", "commodity", "cocoa", "palm oil", "coffee", "cotton"], "higher exposure to low-wage work, complex supply chains, migrant or seasonal labour, supplier pressure, audit limitations and worker-voice challenges"),
    ("Medium", ["bank", "finance", "insurance", "telecom", "digital", "aviation", "airline", "transport", "chemical", "energy", "infrastructure", "manufacturing", "industrial", "technology", "utility", "gas", "logistics", "textile"], "meaningful exposure to customer rights, contractor management, responsible procurement, safety, data/privacy or affected-community expectations"),
...
STANDARDS = [
    {"name": "EU UCPD / Directive 2024/825", "use": "Flag broad or insufficiently substantiated social/sustainability claims such as ethical, fair, responsible, inclusive or positive impact."},
...
def standards_for_claim(t):
    x = (t or "").lower()
    if "human" in x or "labour" in x or "labor" in x:
        return ["CSRD/ESRS S1-S2", "CSDDD", "OECD Guidelines", "UNGPs", "ILO"]
    if "supply" in x or "supplier" in x:
        return ["CSRD/ESRS S2", "CSDDD", "OECD Guidelines", "UNGPs", "ILO"]
    if "diversity" in x or "inclusion" in x:
        return ["CSRD/ESRS S1", "ILO", "GRI"]
    if "customer" in x or "accessibility" in x:
        return ["CSRD/ESRS S4", "OECD Guidelines", "GRI"]
    if "worker" in x or "safety" in x:
        return ["CSRD/ESRS S1", "ILO", "GRI"]
    if "impact" in x or "community" in x:
        return ["CSRD/ESRS S3", "UNGPs", "OECD Guidelines", "GRI"]
    return ["CSRD/ESRS S1-S4", "OECD Guidelines", "GRI"]
...
def clean_excerpt(text, trig):
    ...
    if len(out) < 45:
        out = " ".join(text[max(0, i - 130):min(len(text), i + len(trig) + 240)].split())
...
def source_credibility(result):
    ...
    if any(x in text for x in ["oecd", "europa.eu", ".gov", "ilo.org", "ohchr.org", "un.org", "regulator", "authority", "commission", "court"]):
        return "High"
    if any(x in text for x in ["reuters", "bbc", "ft.com", "guardian", "bloomberg", "amnesty", "human rights watch", "oxfam", "clean clothes", "ngo"]):
        return "Medium-high"
    if any(x in text for x in ["blog", "forum", "opinion"]):
        return "Low"
    return "Medium"
...
def compact_sources(results, limit=6):
    ...
    for r in results[:limit]:
        ...
        cat = "Public web"
        if any(v in txt for v in ["ngo", "amnesty", "oxfam", "human rights watch", "clean clothes"]):
            cat = "NGO / civil society"
        elif any(v in txt for v in ["gov", "europa", "regulator", "authority", "commission", "oecd", "ncp"]):
            cat = "Government / regulator"
        elif any(v in txt for v in ["lawsuit", "court", "legal", "complaint"]):
            cat = "Legal / complaint"
        elif any(v in txt for v in ["reuters", "ft.com", "bbc", "guardian", "press"]):
            cat = "Press"
...
def fetch_html(url):
    ...
    req = Request(url, headers={"User-Agent": "Mozilla/5.0 SocialClaimRiskScan/21.0", "Accept": "text/html,application/xhtml+xml"})
...
def relevant(h):
    h = h.lower()
    return any(k in h for k in ["sustain", "responsib", "people", "human", "rights", "divers", "inclusion", "supplier", "ethic", "impact", "community", "accessibility", "safety", "annual", "report", "esg", "social"])
...
def crawl(url):
    ...
    for href in links:
        ...
        if same_domain(full, host) and relevant(full) and full not in cands and full != url:
            cands.append(full)
    for link in cands[:5]:
        try:
...
def crawl_with_fallback(original_url):
    fallback_note = ""
    try:
        return crawl(original_url), original_url, fallback_note
    except Exception as first_error:
        fallback_url = replace_tld_with_be(original_url)
        if not fallback_url:
            raise first_error
        try:
            result = crawl(fallback_url)
            fallback_note = f"The original .com website was not accessible. The scan was automatically performed on {fallback_url}."
            return result, fallback_url, fallback_note
        except Exception as second_error:
            raise ValueError(
                "The original .com website could not be accessed and the .be fallback also failed. "
                f"Original error: {first_error}. Fallback error: {second_error}."
            )


def infer_company(url, text):
    combo = (url + " " + text[:5000]).lower().replace(" ", "")
    loose_combo = (url + " " + text[:5000]).lower()
    for k, p in PROFILES.items():
        if k in combo or k in loose_combo:
...
def infer_sector(company, text):
    if company.get("sector_risk"):
        level = company["sector_risk"]
        basis = "recognised company/sector profile"
    else:
        level = "Medium"
        basis = "default medium exposure"
        lower = (company.get("sector", "") + " " + text[:15000]).lower()
        for lvl, terms, risks in SECTOR_RULES:
            hits = [t for t in terms if t in lower]
            if hits:
                level = lvl
                basis = "matched terms: " + ", ".join(hits[:5])
                break
...
def external(company):
    if not TAVILY_API_KEY:
        return {"enabled": False, "summary": "External public-source search is not enabled because TAVILY_API_KEY is not configured.", "results": [], "compact_sources": []}
    qs = [
        company + " human rights labour rights controversy NGO report",
        company + " workers union strike discrimination lawsuit",
        company + " supplier supply chain forced labour child labour",
        company + " regulator complaint customer protection accessibility",
        company + " social responsibility criticism press investigation",
    ]
    allr = []
    seen = set()
    for q in qs:
        try:
            ...
            allr.append({"title": "External search query failed", "url": "", "content": str(e)[:240], "score": 0, "query": q})
...
def summarise_ext(results):
    if not results:
        return "No external public-source results were returned."
...
def infer_context(company, text, ext):
    combo = (company.get("context", "") + " " + text[:20000] + " " + ext.get("summary", "")).lower()
    level = "Medium" if "No recognised" not in company.get("context", "") else "Low"
    ...
    if any(t in combo for t in high) and level == "Low":
        level = "Medium"
    elif any(t in combo for t in med) and level == "Low":
        level = "Medium"
...
def evidence_quality_credit(findings, text=""):
    evidence_terms = [
        "policy", "target", "kpi", "indicator", "baseline", "action plan", "progress", "grievance", "remediation", "corrective action", "audit scope", "worker voice", "independent verification", "coverage", "scope", "limitations", "due diligence", "supplier audit", "complaints mechanism", "remedy", "reported annually", "annual report", "metrics", "performance indicators", "corrective-action",
    ]
    combined = (text[:50000] + " " + " ".join((f.get("claim", "") + " " + f.get("issue", "") + " " + f.get("rewrite", "")) for f in findings)).lower()
    hits = [term for term in evidence_terms if term in combined]
    if len(hits) >= 8:
        return 10
    if len(hits) >= 5:
        return 7
...
def external_relevance_score(findings, external_research):
    ...
    text = " ".join((r.get("title", "") + " " + r.get("content", "") + " " + r.get("url", "")).lower() for r in external_research.get("results", []))
    if not text.strip():
...
def guidance(findings, sector):
    ...
    if sector["level"] == "High":
        txt += " Because this sector has higher structural exposure, avoid generic wording such as 'ethical', 'fair', 'responsible' or 'for everyone' unless scope, coverage, limitations and remediation evidence are explicit."
    return txt
...
def evidence_checklist(f):
    ...
    if "supply" in t or "supplier" in t:
        return base + ["supplier-tier coverage", "audit/assessment methodology", "worker voice or grievance mechanism", "corrective-action closure rate", "remediation examples"]
    if "human" in t or "labour" in t or "labor" in t:
        return base + ["salient human-rights risk assessment", "due-diligence steps", "grievance channels", "tracking of outcomes", "remedy process"]
    if "diversity" in t or "inclusion" in t:
        return base + ["workforce diversity data", "baseline and targets", "pay-equity data where relevant", "employee feedback", "progress against action plan"]
    if "customer" in t or "accessibility" in t:
        return base + ["customer outcome metrics", "complaint data", "vulnerable-customer safeguards", "accessibility measures", "remedy process"]
    if "safety" in t or "worker" in t:
        return base + ["incident rates", "contractor coverage", "training data", "safety audit results", "corrective actions"]
...
def build_red_flags(findings, ext, sector, context):
    flags = []
    if any(f.get("risk") == "High" for f in findings):
        flags.append("Broad or high-sensitivity social claims appear on the website and may require stronger substantiation.")
    if any(("supplier" in f.get("type", "").lower() or "supply" in f.get("type", "").lower()) for f in findings):
        flags.append("Supply-chain wording should be checked against supplier coverage, audit quality, worker voice and remediation evidence.")
    if any(("human" in f.get("type", "").lower() or "labour" in f.get("type", "").lower() or "labor" in f.get("type", "").lower()) for f in findings):
        flags.append("Human-rights or labour-rights claims require evidence of due diligence, grievance channels and remedy.")
    if ext.get("enabled") and ext.get("results"):
        flags.append("External public-source signals were found and should be verified before relying on the company's wording.")
    if sector.get("level") == "High":
        flags.append("The sector has structurally higher exposure to labour, supplier, worker or vulnerable-stakeholder issues.")
    if context.get("level") in ["High", "Very high"]:
        flags.append("Company/context sensitivity is elevated and should be considered in investor due diligence.")
    if not flags:
        flags.append("No major investor red flag was detected from available website and public-source signals, but manual verification remains necessary.")
    return flags[:6]
...
def build_company_action_plan(findings, sector, ext):
    ...
    if any(f.get("risk") == "High" for f in findings):
        actions.append({"priority": "Priority 2", "title": "Rewrite broad or absolute claims", "action": "Replace generic wording such as ethical, responsible, fair, fully, all or for everyone with scoped, measurable and evidence-backed wording."})
    ...
    if any(("supplier" in f.get("type", "").lower() or "supply" in f.get("type", "").lower()) for f in findings):
        actions.append({"priority": "Priority 4", "title": "Strengthen supplier-claim substantiation", "action": "Add supplier-tier coverage, audit methodology, corrective-action closure rates, grievance channels and remediation evidence."})
    if ext.get("enabled") and ext.get("results"):
        actions.append({"priority": "Priority 5", "title": "Review external-source signals", "action": "Check whether public-source signals contradict or qualify website claims and record how the company addresses them."})
    return actions[:6]
...
def build_engagement_questions(findings, ext):
    ...
    for f in findings[:4]:
        t = f.get("type", "").lower()
        if "supply" in t or "supplier" in t:
            qs.append("What percentage of suppliers and supplier tiers is covered by due diligence, and what corrective actions were closed during the reporting period?")
        elif "human" in t or "labour" in t or "labor" in t:
            qs.append("What salient human-rights risks were identified, and what grievance and remediation mechanisms are available to affected workers or communities?")
        elif "diversity" in t or "inclusion" in t:
            qs.append("Which workforce diversity, inclusion or pay-equity indicators support the claim, and what progress was made versus baseline?")
        elif "customer" in t or "accessibility" in t:
            qs.append("Which customer outcome, accessibility, complaint and remedy indicators support the claim?")
        else:
            qs.append("What evidence, methodology and reporting boundary support this claim, and what limitations should be disclosed?")
    if ext.get("enabled") and ext.get("results"):
        qs.append("How has management assessed and responded to the external public-source signals identified in this scan?")
    out = []
    for q in qs:
        if q not in out:
            out.append(q)
...
def build_confidence(pages, ext, findings):
    ...
    if len(pages) >= 3:
        pts += 2
        reasons.append("several company pages were reviewed")
    elif len(pages) >= 1:
        pts += 1
        reasons.append("at least the main company page was reviewed")
    if ext.get("enabled") and len(ext.get("results", [])) >= 5:
        pts += 2
        reasons.append("external public-source search returned several results")
    elif ext.get("enabled"):
        pts += 1
        reasons.append("external public-source search was active")
    else:
        reasons.append("external public-source search was not active")
    if findings and not findings[0].get("type", "").lower().startswith("no major"):
        pts += 1
        reasons.append("claim-level signals were detected")
    return {"level": "High" if pts >= 5 else "Medium" if pts >= 3 else "Low", "reasons": reasons}


def split_scores(findings, sector, context, external_modifier, evidence_credit=0):
    claim = max(f.get("claim_score", 0) for f in findings)
    return {
        "claim_wording_risk": min(100, round(claim * 0.70)),
        "external_context_risk": min(100, round((external_modifier or 0) * 4 + {"Low": 5, "Medium": 22, "High": 45, "Very high": 65}.get(context.get("level", "Low"), 5))),
        "sector_baseline_risk": {"Low": 10, "Medium": 35, "High": 60}.get(sector.get("level", "Medium"), 35),
...
def analyse_url(raw):
    ...
    (txt, pages), final_url, fallback_note = crawl_with_fallback(original_url)
    comp = infer_company(final_url, txt)
    ext = external(comp["company"])
    ...
    return {
        ...
        "analysis_date": datetime.datetime.now(datetime.timezone.utc).isoformat(timespec="seconds"),
        "overall_score": score,
        ...
        "evidence_credit": evidence_credit,
        "split_scores": split_scores(fs, sec, ctx, external_modifier, evidence_credit),
        "claim_inventory": build_claim_inventory(fs),
        "investor_red_flags": build_red_flags(fs, ext, sec, ctx),
        ...
        "disclaimer": "Indicative first-pass assessment only. It is not legal advice and not a finding that social washing occurred. External search results are review signals that require verification.",
        "analysed_text_excerpt": txt[:2200],
        "quality_improvements": [
            "Use claim-specific wording rather than broad reassurance language.",
            "Connect each claim to scope, metrics, reporting period and limitations.",
            "For supplier, human-rights or worker claims, add due-diligence, grievance and remediation evidence.",
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _json(self, d, status=200):
        self._send(json.dumps(d, ensure_ascii=False, indent=2), "application/json; charset=utf-8", status)
    ...
    def do_GET(self):
        if self.path == "/" or self.path.startswith("/?"):
            frontend = APP_DIR / "frontend.html"
            if frontend.exists():
                self._send(frontend.read_text(encoding="utf-8"))
            else:
                self._send("<h1>Social Washing Scan</h1><p>Frontend file not found. API is available at /api/health and /api/scan/url.</p>")
        elif self.path == "/api/health":
            self._json({"status": "ok", "version": APP_VERSION, "version_notes": VERSION_NOTES, "ai_configured": bool(OPENAI_API_KEY), "tavily_configured": bool(TAVILY_API_KEY)})
        else:
